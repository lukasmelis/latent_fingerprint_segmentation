...
ROOT_DIR = 'maskrcnn_latent/mrcnn_root'
...
dataset_val.load_data('./maskrcnn_latent/data/val/annotations.json', './maskrcnn_latent/data/')
dataset_val.prepare()
...
def main(image_path):
    ...
    model_path = "maskrcnn_latent/mask_rcnn_latent.h5"
    ...
    if (np.amax(img) > 255):
        ...
        img = ((img / ratio) - 1).astype('uint8')
    img = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
    img_arr = np.array(img)
    results = model.detect([img_arr], verbose=1)
    r = results[0]
    end_time = time.time()
    print("cas Mask R-CNN(sec): ", round(end_time - start_time, 2))
    return visualize.display_instances(img, r['rois'], r['masks'], r['class_ids'],
